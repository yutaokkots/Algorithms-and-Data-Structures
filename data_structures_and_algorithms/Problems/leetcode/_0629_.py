...
class Solution:
    """Solution class for LC629 adapted from user @Recode_-1"""
        ...
        for i in range(1, n + 1):
            for j in range(k + 1):
                if j == 0:
                    ...
                    value = (dp[i-1][j] + modulo - 
                            (dp[i-1][j-1]
                            if j - i >=0 else 0)) % modulo
                    dp[i][j] = (dp[i][j - 1] + value) % modulo
        return (dp[n][k] + modulo - (dp[n][k - 1] if k > 0 else 0)) % modulo